...
impl Database {
    ...
    pub async fn add_host(&self, hostname: &str, ip: &str) -> Result<(), sqlx::Error> {
        sqlx::query!("INSERT INTO hosts (hostname, ip) VALUES (?, ?)", hostname, ip).execute(&self.pool).await?;
        Ok(())
    }
    ...
    pub async fn list_hosts(&self) -> Result<Vec<DBHost>, sqlx::Error> {
        let hosts =
            sqlx::query_as!(DBHost, "SELECT id as 'id!', hostname, ip FROM hosts").fetch_all(&self.pool).await?;
        Ok(hosts)
    }
